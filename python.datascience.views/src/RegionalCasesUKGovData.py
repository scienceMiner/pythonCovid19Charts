...
def barPlot():
    data = pd.read_csv("/Users/ethancollopy/Downloads/London_2021-01-07.csv"  , index_col='date' ) 
    #data['cumDeaths28DaysByDeathDate'] = data['cumDeaths28DaysByDeathDate'].diff()
    #print(data)
    data=data.iloc[::-1] # reverse the date
    mergedDF = pd.concat([data], axis=1 ) # axis 1 is for the same index 
    #newDF = mergedDF.drop(mergedDF.index[[1..40]])
    mergedDF = mergedDF.drop(mergedDF.index[0:20])
   # print('index to drop' + mergedDF.index[[1,40]])
    print(mergedDF)
    #ax = mergedDF.plot.bar(x='date',y='cumCasesByPublishDate')
    ax = mergedDF.plot.bar(y=['cumDeaths28DaysByDeathDate'])
    ax.set_title(" UK COVID-19 Cases & Deaths" )
    ax.set_xlabel('Date')   
    ax.set_ylabel('Number of Cases (m)')
    ax.set_facecolor('gainsboro')
    #ax.ticklabel_format(style='plain')
    ax.get_yaxis().set_major_formatter(tick.FuncFormatter(lambda x, p: format(int(x), ','))) # stop format truncation
