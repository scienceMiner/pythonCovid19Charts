...
def plot(cx , data, Country, daysToSkip):
    ...
    x2 = np.array(x)
    z4 = np.polyfit(x2, y2, 5)
    p4 = np.poly1d(z4)
    xx_out = np.linspace(x2.min(), x2.max()+90, 300)
    #y3 = np.polyval(z4,x2) 
    legend = "{} Cases to date".format(Country)
    legendExtrapolated = "{} Cases Naive Projection".format(Country)
    dd_out = mdates.num2date(xx_out) # CONVERT BACK TO DATES 
    cx.plot(dd_out, p4(xx_out), label=legendExtrapolated) # PLOT EXTRAPOLATED VALUES    
...
cx.legend()
cx.set_xlabel('Date')
cx.set_ylabel('Number of Cases')
cx.set_title('Number of Cases - Naive Extrapolation - Indian cases exceed Brazil in October')
